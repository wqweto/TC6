...
## [src/mdWinApi.bas](src/mdWinApi.bas) — winsqlite3.dll API layer
...
- **283** functions declared (`Public Declare PtrSafe … Lib "winsqlite3"`).
...
- **`sqlite3_int64` → `Currency`** (not `LongLong`, which VB6 lacks: "User-defined
  type not defined"). `Currency` is an 8-byte slot in both x86 and x64; put the
  raw 64-bit value in via a VarPtr/CopyMemory helper (wrapper concern).
...
A Std-EXE VB6 project that compiles `src\mdWinApi.bas` and exercises it at
runtime, built and run headless with the OS's `VB6.EXE` (`/make ... /out`).
...
- [x] SQLite backend binding: `mdWinApi.bas` with all winsqlite3.dll exports
      declared (StdCall, verified) plus core constants.
- [x] VB6 (x86) + tB dual-target compatibility of `mdWinApi.bas`.
